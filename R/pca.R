...
pca.biplot= function(dataset, pca.result, cex = 0.8, legend.cex = 0.8, x.colors = 1, legend.place = "topright", ...) {
  ...
  if (x.flag){
	...
	legend(legend.place, pch=c(1,3), inset=c(-0.2,0), levels(dataset$metadata[, x.colors]), cex=legend.cex, fill = sort(as.integer(factor(levels(dataset$metadata[, x.colors]))))) 
  }
}
